# bash_alias_suse.sh
#set -x
printf "\n  %s\n" "+++ Start source file bash_aliases_suse.sh +++"
...
alias ll='ls -lhF --color=auto'    # t sort date and time
alias lrt='ls -art --color=auto'   # l long format permission
alias llrt='ls -alrt --color=auto' # l long format permission
alias lla='ls -alF --color=auto'   # a for hidden files
...
#alias more="\less -i"
#alias m="\less -X -I"
#alias m='less -FSRXc' # Preferred 'less' implementation
alias m='\less -FX -I' # Preferred 'less' implementation
...
alias l.l='ls -dl .* --color=auto'
alias l.lr='ls -rtl -d .* --color=auto'
alias l.r='ls -rt -d .* --color=auto'

alias shq="/usr/local/bin/showquota -h | head -15" # universaj

#export TRASH_CLI="$HOME/trash-cli"    # see bash_export
#export TRASH_DIR=set_trash_dir        # see bash_export

alias trash-put="${TRASH-CLI}/trash-put  --trash-dir ${TRASH_DIR} "
alias trash-list="${TRASH-CLI}/trash-list --trash-dir ${TRASH_DIR} "
alias trash-restore="${TRASH-CLI}/trash-restore  --trash-dir ${TRASH_DIR} "
alias trash-rm="${TRASH-CLI}/trash-rm  --trash-dir ${TRASH_DIR} "
alias trash-empty="${TRASH-CLI}/trash-empty-rm  --trash-dir ${TRASH_DIR} "

NOBACKUP=""/discover/nobackup/${USERNAME}""
alias gon="cd ${NOBACKUP}; ls"
alias gonr="cd ${NOBACKUP}; ls -lart"
alias gome="cd ${NOBACKUP}/MEs; ls"
alias gor="cd ${NOBACKUP}/RUNS_ME; ls"

#alias goa="cd $ARCHIVE"
alias ggiss="cd /discover/nobackup/projects/giss_ana/users/ntausnev; ls"
alias ggissr="cd /discover/nobackup/projects/giss_ana/users/ntausnev/RUNS_ME/; ls"
...
alias gon="cd ${NOBACKUP}; ls"
alias gonr="cd ${NOBACKUP}; ls -lart"
alias gopbs="cd /discover/nobackup/ntausnev/RUNS_ME/z_pbs; ls -lart"

alias mj="ps -ef | head -1 ; ps -ef | grep ^${USERNAME} | sort -k7 -nr | grep -v sort | grep -v grep | grep -v ps"
alias mybjobs="qstat -a | grep ${USERNAME}"
alias mbj="qstat -a | grep ${USERNAME}"

alias myspace="du -sh * | sort -rn | more"
#alias how_many_files="find . -type d | xargs ls -1 | \
...
printf "\n  %s\n" "+++ Ended source file bash_aliases_suse.sh +++"
###################################################################################
